...
from utils import TFCSVRecordReader
...
class DataVisualizer:
    ...
    Methods:
        __init__(self, input_file='', input_dir='', output_dir='', data_input_format='csv', logger=None):
    ...
    def __init__(self, input_file='', input_dir='', output_dir='', data_input_format='csv', encoding='', verbose='INFO'):
        """
        ...
        if not os.path.exists(input_file) and os.path.exists(input_dir):
        ...
        if os.path.exists(input_dir):
            ...
            if data_input_format != 'csv':
                self.tf_dataset_files = glob.glob(f'{self.dataset_path}/*.tfrecord')
            else:
                self.csv_dataset_files = glob.glob(f'{self.dataset_path}/*.csv')
            
            # Check for valid files
            if len(self.csv_dataset_files) == 0 and len(self.tf_dataset_files) == 0:
                raise ValueError("The input directory is empty!")
           
            self.tfrecord_processor = TFCSVRecordReader(input_path=self.dataset_path, 
                                                           data_pipeline_file_format=data_input_format)
            ...
            raise ValueError(f"Please provide valid input! input_dir: {input_dir},  input_file:{input_file}")
        
         # Instantiate TFCSVRecordReader for further processing
        self.tfrecord_processor = TFCSVRecordReader(input_file=input_file, input_path=input_dir,
                                                        data_input_format=data_input_format, encoding=encoding)
        self.input_file = input_file

        self.logger.debug("input_file : ", input_file)
        self.logger.debug("input_dir : ", input_dir)
        self.logger.debug("output_dir : ", output_dir)
    ...
    def read_tfrecord_as_df(self, tfrecord_file):
        ...
        for landmarks, phrase in dataset:
            landmarks_list.append(landmarks.numpy())
            phrase_list.append(phrase.numpy().decode('utf-8'))
            
        landmarks = pd.DataFrame(landmarks_list)
        return landmarks, phrase_list[0]
    ...
    def visualize_data(self, csv_file=None, tfrecord_file=None, tf_file_index=-1, csv_file_index=-1, animation_name='', write=False, output_format='.gif'):
        """
